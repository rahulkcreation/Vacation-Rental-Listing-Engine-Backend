document.addEventListener("DOMContentLoaded", function () {
  ...
  function lebShowLoading() {
    if (document.getElementById("leb-tl-loader")) {
      return;
    }
  ...
  function lebRenderCards(items) {
    ...
    items.forEach(function (row, index) {
      ...
      html += [
        ...
        '    <input type="checkbox" class="leb-tl-checkbox leb-tl-item-checkbox" value="' +
          row.id +
          '"' +
          (isSelected ? " checked" : "") +
          ">",
        ...
        '    <span class="leb-tl-card-value">' +
          lebEscHtml(lebFormatDate(row.updated_at)) +
          "</span>",
        "  </div>",
        ...
        "      Edit",
        "    </a>",
        '    <button class="leb-tl-delete-btn" data-id="' +
          row.id +
          '" data-name="' +
          row.name +
          '">',
    ...
    domCardsList
      .querySelectorAll(".leb-tl-item-checkbox")
      .forEach(function (cb) {
        cb.addEventListener("change", function () {
          ...
          if (this.checked) {
            ...
            lebState.selectedIds = lebState.selectedIds.filter(function (sid) {
              return sid !== id;
            });
          }
          lebUpdateBulkBar();
  ...
  window.lebBulkDelete = function () {
    ...
    if (typeof LEB_Confirm === "undefined") {
      if (
        confirm("Delete " + lebState.selectedIds.length + " selected types?")
      ) {
    ...
    LEB_Confirm.show({
      ...
      message:
        ...
        lebState.selectedIds.length +
        " selected items? This cannot be undone.",
  ...
  function lebRenderPagination(total, page, perPage) {
    ...
    // Previous button.
    html +=
      '<button class="leb-pg-btn" id="leb-pg-prev" aria-label="Previous page"' +
      (page <= 1 ? " disabled" : "") +
      ">";
    html +=
    ...
    for (var i = windowStart; i <= windowEnd; i++) {
      html +=
        '<button class="leb-pg-btn' +
        (i === page ? " leb-pg-active" : "") +
        '" data-page="' +
        i +
        '">' +
        i +
    ...
    // Next button.
    html +=
      '<button class="leb-pg-btn" id="leb-pg-next" aria-label="Next page"' +
    ...
    if (domPagControls) {
      ...
      // Bind page buttons.
      domPagControls
        .querySelectorAll(".leb-pg-btn[data-page]")
