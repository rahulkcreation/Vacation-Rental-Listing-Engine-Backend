document.addEventListener( 'DOMContentLoaded', function () {
    ...
    function lebLocShowLoading() {
        if ( document.getElementById( 'leb-loc-loader' ) ) { return; }
    ...
    function lebLocRenderCards( items ) {
        ...
        items.forEach( function ( row, index ) {
            ...
            var svgCell = '<img class="leb-loc-svg-preview" src="' + lebLocEscHtml( finalSvgPath ) + '" alt="' + lebLocEscHtml( row.name ) + ' icon" width="24" height="24" loading="lazy" onerror="this.onerror=null;this.src=\'' + defaultSvgPath + '\';">';

            html += [
                '<div class="leb-loc-card" data-id="' + row.id + '">',
                '  <div class="leb-loc-checkbox-wrap">',
                '    <input type="checkbox" class="leb-loc-checkbox leb-loc-item-checkbox" value="' + row.id + '"' + ( isSelected ? ' checked' : '' ) + '>',
                '  </div>',
                '  <div class="leb-loc-card-row">',
                '    <span class="leb-loc-card-label">S.No</span>',
                '    <span class="leb-loc-card-value leb-loc-s-no">' + sno + '</span>',
                '  </div>',
                '  <div class="leb-loc-card-row">',
                '    <span class="leb-loc-card-label">Name</span>',
                '    <span class="leb-loc-card-value">' + lebLocEscHtml( row.name ) + '</span>',
                '  </div>',
                '  <div class="leb-loc-card-row">',
                '    <span class="leb-loc-card-label">Slug</span>',
                '    <span class="leb-loc-card-value">' + lebLocEscHtml( row.slug ) + '</span>',
                '  </div>',
                '  <div class="leb-loc-card-row">',
                '    <span class="leb-loc-card-label">SVG Icon</span>',
                '    <span class="leb-loc-card-value leb-loc-svg-cell">' + svgCell + '</span>',
                '  </div>',
                '  <div class="leb-loc-card-row">',
                '    <span class="leb-loc-card-label">Updated</span>',
                '    <span class="leb-loc-card-value">' + lebLocEscHtml( lebLocFormatDate( row.updated_at ) ) + '</span>',
                '  </div>',
                ...
                '      Edit',
                '    </a>',
                '    <button class="leb-loc-delete-btn" data-id="' + row.id + '" data-name="' + lebLocEscHtml( row.name ) + '">',
                '      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">',
        ...
        domCardsList.querySelectorAll( '.leb-loc-item-checkbox' ).forEach( function ( cb ) {
            cb.addEventListener( 'change', function () {
                var id = parseInt( this.value );
                if ( this.checked ) {
                    if ( ! lebLocState.selectedIds.includes( id ) ) { lebLocState.selectedIds.push( id ); }
                } else {
                    lebLocState.selectedIds = lebLocState.selectedIds.filter( function ( sid ) { return sid !== id; } );
                }
                lebLocUpdateBulkBar();
            } );
        } );
        ...
        /* Bind delete buttons */
        domCardsList.querySelectorAll( '.leb-loc-delete-btn' ).forEach( function ( btn ) {
            btn.addEventListener( 'click', function () {
                lebLocConfirmDelete( this.getAttribute( 'data-id' ), this.getAttribute( 'data-name' ) );
            } );
        } );
    ...
    window.lebLocUpdateBulkBar = function () {
        ...
        if (bulkBar && selectedCount) {
            if ( lebLocState.selectedIds.length > 0 ) {
                ...
                selectedCount.textContent = lebLocState.selectedIds.length + ' selected';
            } else {
    ...
    window.lebLocBulkDelete = function () {
        if ( lebLocState.selectedIds.length === 0 ) { return; }

        if ( typeof LEB_Confirm === 'undefined' ) {
            if ( confirm( 'Delete ' + lebLocState.selectedIds.length + ' selected locations?' ) ) {
        ...
        LEB_Confirm.show( {
            ...
            message     : 'Are you sure you want to delete ' + lebLocState.selectedIds.length + ' selected locations? This cannot be undone.',
            confirmText : 'Delete All',
            type        : 'leb-warning',
            onConfirm   : function () { lebLocPerformBulkDelete(); }
        } );
    };
    ...
    function lebLocRenderPagination( total, page, perPage ) {
        ...
        var html = '';
        html += '<button class="leb-loc-pg-btn" id="leb-loc-pg-prev" aria-label="Previous page"' + ( page <= 1 ? ' disabled' : '' ) + '>';
        ...
        for ( var i = windowStart; i <= windowEnd; i++ ) {
            html += '<button class="leb-loc-pg-btn' + ( i === page ? ' leb-loc-pg-active' : '' ) + '" data-page="' + i + '">' + i + '</button>';
        }

        html += '<button class="leb-loc-pg-btn" id="leb-loc-pg-next" aria-label="Next page"' + ( page >= totalPages ? ' disabled' : '' ) + '>';
        ...
        if (domPagControls) {
            ...
            domPagControls.querySelectorAll( '.leb-loc-pg-btn[data-page]' ).forEach( function ( btn ) {
                btn.addEventListener( 'click', function () {
                    lebLocState.currentPage = parseInt( this.getAttribute( 'data-page' ), 10 );
                    lebLocFetchLocations();
                } );
            } );
        }
        ...
        if ( prevBtn ) {
            prevBtn.addEventListener( 'click', function () {
                if ( lebLocState.currentPage > 1 ) { lebLocState.currentPage--; lebLocFetchLocations(); }
            } );
        }
        if ( nextBtn ) {
            nextBtn.addEventListener( 'click', function () {
                if ( lebLocState.currentPage < totalPages ) { lebLocState.currentPage++; lebLocFetchLocations(); }
            } );
        }
    }
    ...
    function lebLocConfirmDelete( id, name ) {
        if ( typeof LEB_Confirm === 'undefined' ) {
            if ( confirm( 'Are you sure you want to delete "' + name + '"?' ) ) { lebLocPerformDelete( id ); }
            return;
        }
        LEB_Confirm.show( {
            ...
            cancelText  : 'Cancel',
            type        : 'leb-warning',
            onConfirm   : function () { lebLocPerformDelete( id ); }
        } );
    }
    ...
    function lebLocFetchLocations() {
        if ( lebLocState.isLoading ) { return; }
        ...
        fetch( ajaxUrl, { method: 'POST', body: formData, credentials: 'same-origin' } )
            ...
            .then( function ( data ) {
                ...
                if ( data.success && data.data ) {
                    ...
                    lebLocRenderPagination( result.total, lebLocState.currentPage, lebLocState.perPage );
                } else {
                    lebLocRenderEmpty( 'Failed to load locations.' );
                    if ( typeof LEB_Toaster !== 'undefined' ) {
                        LEB_Toaster.show( ( data.data && data.data.message ) ? data.data.message : 'Error loading locations.', 'error' );
                    }
                }
            } )
            .catch( function () {
    ...
    if (domSearchInput) {
        ...
        domSearchInput.addEventListener( 'blur',  function () {
            if (domSearchWrap) domSearchWrap.classList.remove( 'leb-loc-search-focused' );
        } );
    }
