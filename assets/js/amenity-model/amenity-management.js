document.addEventListener("DOMContentLoaded", function () {
  ...
  function lebAmenExtractSvgSrc(svgRaw) {
  ...
  function lebAmenShowLoading() {
    if (document.getElementById("leb-amen-loader")) {
      return;
    }
  ...
  function lebAmenRenderCards(items) {
    ...
    items.forEach(function (row, index) {
      var sno =
        (lebAmenState.currentPage - 1) * lebAmenState.perPage + index + 1;
      ...
      var finalSvgPath = lebAmenExtractSvgSrc(row.svg_path);
      var svgCell =
        '<img class="leb-am-svg-preview" src="' +
        lebAmenEscHtml(finalSvgPath) +
        '" alt="' +
        lebAmenEscHtml(row.name) +
      ...
      html += [
        ...
        '    <input type="checkbox" class="leb-am-checkbox leb-am-item-checkbox" value="' +
          row.id +
          '"' +
          (isSelected ? " checked" : "") +
          ">",
        ...
        '    <span class="leb-am-card-value">' +
          lebAmenEscHtml(row.name) +
          "</span>",
        "  </div>",
        '  <div class="leb-am-card-row">',
        '    <span class="leb-am-card-label">SVG Icon</span>',
        '    <span class="leb-am-card-value leb-am-svg-cell">' +
          svgCell +
          "</span>",
        "  </div>",
        '  <div class="leb-am-card-row">',
        ...
        '    <span class="leb-am-card-value">' +
          lebAmenEscHtml(lebAmenFormatDate(row.updated_at)) +
          "</span>",
        "  </div>",
        ...
        "      Edit",
        "    </a>",
        '    <button class="leb-am-delete-btn" data-id="' +
          row.id +
          '" data-name="' +
          lebAmenEscHtml(row.name) +
          '">',
    ...
    domCardsList
      .querySelectorAll(".leb-am-item-checkbox")
      .forEach(function (cb) {
        cb.addEventListener("change", function () {
          var id = parseInt(this.value);
          if (this.checked) {
            if (!lebAmenState.selectedIds.includes(id)) {
              lebAmenState.selectedIds.push(id);
            }
          } else {
            lebAmenState.selectedIds = lebAmenState.selectedIds.filter(
              function (sid) {
                return sid !== id;
              },
            );
    ...
    if (bulkBar && selectedCount) {
      if (lebAmenState.selectedIds.length > 0) {
        ...
        selectedCount.textContent =
          lebAmenState.selectedIds.length + " selected";
      } else {
  ...
  var domSelectAll = document.getElementById("leb-amen-select-all");
  if (domSelectAll) {
  ...
  window.lebAmenBulkDelete = function () {
    ...
    if (typeof LEB_Confirm === "undefined") {
      if (
        confirm(
          "Delete " + lebAmenState.selectedIds.length + " selected amenities?",
        )
      ) {
    ...
    LEB_Confirm.show({
      ...
      message:
        ...
        lebAmenState.selectedIds.length +
        " selected amenities? This cannot be undone.",
  ...
  function lebAmenRenderPagination(total, page, perPage) {
    ...
    var html = "";
    html +=
      '<button class="leb-am-pg-btn" id="leb-amen-pg-prev" aria-label="Previous page"' +
      (page <= 1 ? " disabled" : "") +
      ">";
    html +=
    ...
    for (var i = windowStart; i <= windowEnd; i++) {
      html +=
        '<button class="leb-am-pg-btn' +
        (i === page ? " leb-am-pg-active" : "") +
        '" data-page="' +
        i +
        '">' +
        i +
    ...
    html +=
      '<button class="leb-am-pg-btn" id="leb-amen-pg-next" aria-label="Next page"' +
    ...
    if (domPagControls) {
      ...
      domPagControls
        .querySelectorAll(".leb-am-pg-btn[data-page]")
        .forEach(function (btn) {
          btn.addEventListener("click", function () {
            lebAmenState.currentPage = parseInt(
    ...
    if (prevBtn) {
      prevBtn.addEventListener("click", function () {
        if (lebAmenState.currentPage > 1) {
          lebAmenState.currentPage--;
          lebAmenFetchAmenities();
        }
      });
    }
    if (nextBtn) {
      nextBtn.addEventListener("click", function () {
        if (lebAmenState.currentPage < totalPages) {
          lebAmenState.currentPage++;
          lebAmenFetchAmenities();
        }
      });
    ...
    fetch(ajaxUrl, {
      ...
      .then(function (data) {
        ...
        if (data.success && data.data) {
          ...
          lebAmenRenderPagination(
            result.total,
            lebAmenState.currentPage,
            lebAmenState.perPage,
          );
        } else {
  ...
  if (domSearchInput) {
    ...
    domSearchInput.addEventListener("focus", function () {
      if (domSearchWrap) domSearchWrap.classList.add("leb-am-search-focused");
    });
    domSearchInput.addEventListener("blur", function () {
      if (domSearchWrap)
        domSearchWrap.classList.remove("leb-am-search-focused");
    });
  }
